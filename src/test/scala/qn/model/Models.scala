...
object Models {
  private val node1: Resource = Resource("Service unit 1", 1)
  private val node2: Resource = Resource("Service unit 2", 1)
  private val node3: Resource = Resource("Service unit 3", 1)
  val dd1 = Network("DD1").add(node1).add(OrdersStream("Fixed Input, Instant Output", Distribution.deterministic(1.0),
    NetworkGraph().addTransition(Resource.source, node1).addTransition(node1, Resource.sink)
      .addService(node1, Distribution.deterministic(0.0))))

  val mm1_08 = Network("MM1")
    .add(node1)
    .add(OrdersStream("Exponential Order Flow", Distribution.exp(0.8), NetworkGraph()
